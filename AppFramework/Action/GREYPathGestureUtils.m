...
static CGPoint GREYPointOnCircle(double angle, CGPoint center, CGFloat radius) {
...
static NSArray<NSValue *> *GREYLinearTouchPathForTwistGesture(CGPoint center, CGFloat radius,
                                                              CGFloat startAngle,
                                                              CGFloat angleDelta,
  ...
  for (NSUInteger i = 0; i < numberOfPoints; i++) {
    ...
    CGPoint touchPoint = GREYPointOnCircle(angleAtPoint, center, radius);
    [touchPath addObject:[NSValue valueWithCGPoint:touchPoint]];
  }
  return touchPath;
}
...
NSArray<NSValue *> *GREYTouchPathForTwistGesture(CGPoint center, CGFloat radius, CGFloat startAngle,
  ...
  NSMutableArray *touchPath = [[NSMutableArray alloc] init];
  CGPoint startPoint = GREYPointOnCircle(startAngle, center, radius);
  [touchPath addObject:[NSValue valueWithCGPoint:startPoint]];
  ...
  if (isnan(duration)) {
    ...
    if (totalPoints > 1) {
      ...
      NSArray *additionalPoints = GREYLinearTouchPathForTwistGesture(
  ...
  CGPoint endPoint = GREYPointOnCircle(endAngle, center, radius);
  NSValue *endPointValue = [NSValue valueWithCGPoint:endPoint];
  if (cancelInertia) {
    // To cancel inertia, slow down as approaching the end point. This is done by inserting a series
    // of points between the 2nd last and the last point.
    ...
    CGFloat angleDelta = (endAngle - semifinalArcAngle) / numberOfIntermediatePoints;
    NSArray *additionalPoints = GREYLinearTouchPathForTwistGesture(
